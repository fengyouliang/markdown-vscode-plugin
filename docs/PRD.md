# VS Code 插件需求文档（PRD）：Markdown 单击自动预览（默认右侧双栏预览，可选单栏）
...
1) **打开 Markdown 即自动预览**
- 用户在资源管理器中单击/打开任意 Markdown 文件（如 `README.md`），默认在**右侧（双栏）**打开 Markdown 渲染预览（Preview），左侧保留源码编辑器。
- 可选配置：用户也可以设置为**当前编辑组（单栏）**打开预览，获得“点击即阅读”的单栏体验。
...
### 5.2 布局规则（单栏/双栏）
- 默认：对当前激活的 Markdown 文档，在**右侧（Beside / side）**打开预览（双栏）。
- 可选：对当前激活的 Markdown 文档，在**当前编辑组（same）**打开预览（单栏）。
...
- `mdAutoPreview.enabled`（boolean，默认 `true`）
  - 是否启用自动预览
- `mdAutoPreview.openLocation`（string，`same` | `side`，默认 `side`）
...
1) 打开任意 `.md` 文件，默认在右侧自动出现渲染预览（双栏预览），左侧保留源码编辑器。
2) 当 `openLocation=same` 时，在当前编辑组打开渲染预览（单栏预览）。
...
## 10. 待确认项（实现前需要你最后拍板）
- 默认策略是否采用：`openLocation=side` + `mode=follow` + `reopenPolicy=always`（更贴近“左侧编辑 + 右侧预览”的原生编辑体验）。
